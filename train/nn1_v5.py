...
def train_model(params):
    dtrain = np.loadtxt(params['train_data_file_path'], delimiter=',', dtype=np.int32)
    dtrain -= [1, 1, 0]
    ...
    embeddings1 = tf.Variable(, name='embeddings1')
    embeddings2 = tf.Variable(, name='embeddings2')
    e_biases = tf.Variable(tf.zeros([params['embedding_size'] * 3]), name='e_biases')
    e1 = tf.nn.embedding_lookup(embeddings1, u1s)
    e2 = tf.nn.embedding_lookup(embeddings2, u2s)
    e3 = tf.mul(e1, e2)
    h0 = tf.concat(1, [e1, e2, e3]) + e_biases

    weights1 = tf.Variable(tf.random_normal([params['embedding_size'] * 3, params['h1_size']], mean = 0, stddev =  1), name='weights1')
...
def main():
    params = {'node_num': 10000, 'round': 10, 'learning_rate': 0.02, 'batch_size': 500, 'learning_rate1': 1, 'learning_rate2': 0.001,
              'embedding_size': 4, 'h1_size': 8, 'h2_size':4,
              'train_data_file_path': '../../data/test/small_train_data_v2_csv', 'model_save_file_path': '../../data/test/model_weights.ckpt'}
