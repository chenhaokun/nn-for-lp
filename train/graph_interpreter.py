...
def show_auc_curve(params):
    ...
    x_list = range(0, params['x_max'], params['x_step'])
    m_list = ['mf', 'pnn1', 'pnn2']
    plot_list = []
    color_list = ['r', 'g', 'b']
...
def pnn_test(params):
    ...
    weighto = tf.Variable(tf.random_normal([params['h4_size'], 1], mean=0, stddev=0.01))
    biaseo = tf.Variable(tf.zeros([1]))
    ys_pre = tf.squeeze(tf.matmul(h4, weighto) + biaseo)# + u1_biases * u2_biases
...
def pnn_test3(params):
    ...
    with tf.Session() as sess:
        ...
        store_dict['embedding_size'] = parasm['embedding_size']
        print 'training pnn1...'
        rmse_v, loss_v, target_loss_v, ys_v, ys_pre_v, accuracy_v = sess.run(
            [rmse, loss, target_loss, ys, tf.sigmoid(ys_pre), accuracy],
            feed_dict={u1s: dtrain[0:800000, 0], u2s: dtrain[0:800000, 1],
        ...
        for i in range(params['round']):
            ...
            if stop_count == 3 or i == (params['round'] - 1):
                break
        store_set = load_obj(params['pnn1_params_test_result_file_path'])
        if store_set == -1:
            store_set = set(store_dict)
        else:
            store_set.add(store_dict)
        store_obj(store_set, params['pnn1_params_test_result_file_path'])
...
def pnn_with_ann_test(params):
    ...
    ys_pre_a = tf.squeeze(tf.matmul(h4_a, weighto_a) + biaseo_a)# + u1_biases * u2_biases
...
def base_exp(params):
    ...
    if 'mf' in params['baseline_set']:
        # svd({'dtrain_file_path': dir + '%s_train_data_v%d' % (data_name, version),
        #      'dtest_file_path': dir + '%s_test_data_v%d' % (data_name, version),
        #      'embedding_size': 20, 'node_num': node_num,
        #      'batch_size': 5000, 'round': 20, 'learning_rate': 5e-3, 'beta': 5e-1})
        mf_with_sigmoid({'dtrain_file_path': dir + '%s_train_data_v%d' % (data_name, version),
                         ...
                         'batch_size': 5000, 'round': 20, 'learning_rate': 5e-3, 'beta': 4e-1,
...
def params_exp(params):
    ...
    pnn_test3({'dtrain_file_path': dir + '%s_train_data_v%d' % (data_name, version),
             'dtest_file_path': dir + '%s_test_data_v%d' % (data_name, version),
             'embedding_size': params['embedding_size'], 'node_num': node_num,
             'h_size_list': params['h_size_list'],
             'batch_size': 5000, 'round': 20, 'learning_rate': 4e-3, 'beta': 4e-4,
...
if __name__ == '__main__':
    ...
    #           'version': 1,
    #           'train_np_rate': 160,
    #           'baseline_set': set(['mf','cn','aa','katz']),  # 'mf','cn','aa','katz'
    #           'pnn1_test': {'round': 16},
    #           'pnn1': False,
    #           'fixed_emb_pnn2': False,
    #           'pnn2_test': {'learning_rate1': 4e-3, 'learning_rate2': 4e-3, 'beta1': 4e-4, 'beta2': 4e-4, 'hop2_np_rate': 160, 'round': 25},
    #           'pnn2': False,
    #           'store_test_result': True,
    #           'show_auc_curve': True})
    # show_pnn1_result({'dtest_a_file_path': '../../data/test1/' +'eu_hop2_train_data_v1',#+ '%s_test_data_v%d' % ('eu', 1),
    #               'pre_model_save_path': '../../data/test1/' + '%s_data_pnn_model_params_saver_v%d' % ('eu', 1),
    #               'embedding_size': 20,
    #               'h1_size': 20, 'h2_size': 20, 'h3_size': 20, 'h4_size': 20})
    ...
    base_exp({'source_file_path': '../../data/test1/openflights_data',
              'version': 1,
              'train_np_rate': 160,
              'baseline_set': set([]),  # 'mf','cn','aa','katz', 'ra'
              'pnn1_test': {},  #{'round': 20},
              'pnn1_test2': {},  # 'round': 16
              'pnn1': False,
              'fixed_emb_pnn2': False,
    ...
    #           'version': 1,
    #           'train_np_rate': 160,
    #           'baseline_set': set(['mf']),  # 'mf','cn','aa','ra', 'katz', 'rwr'
    #           'pnn1_test': {'round': 16},#'round': 16
    #           'pnn1_test2': {},#'round': 16
    #           'pnn1': False,
    #           'fixed_emb_pnn2': False,
    #           'pnn2_test': {'learning_rate1': 4e-3, 'learning_rate2': 4e-3, 'beta1': 4e-4, 'beta2': 4e-4, 'hop2_np_rate': 40, 'round': 25},#'learning_rate1': 4e-3, 'learning_rate2': 4e-3, 'beta1': 4e-4, 'beta2': 4e-4, 'hop2_np_rate': 40, 'round': 25
    #           'pnn2_test2': {},#'learning_rate1': 4e-3, 'learning_rate2': 4e-3, 'beta1': 4e-4, 'beta2': 4e-4, 'hop2_np_rate': 160, 'round': 25
    #           'pnn2': False,
    #           'store_test_result': True,
    #           'show_auc_curve': True})
    ...
    #           'version': 1,
    #           'train_np_rate': 160,
    #           'baseline_set': set(),  # 'mf','cn','aa','ra', 'katz', 'rwr'
    #           'pnn1_test': {},# 'round': 16
    #           'pnn1_test2': {},  # 'round': 16
    #           'pnn1': False,
    #           'fixed_emb_pnn2': False,
    #           'pnn2_test': {},#'learning_rate1': 4e-3, 'learning_rate2': 4e-3, 'beta1': 4e-4, 'beta2': 4e-4, 'hop2_np_rate': 160, 'round': 25, 'random_sample': False
    #           'pnn2_test2': {},
    #           # 'learning_rate1': 4e-3, 'learning_rate2': 4e-3, 'beta1': 4e-4, 'beta2': 4e-4, 'hop2_np_rate': 160, 'round': 25
    #           'pnn2': False,
    #           'store_test_result': True,
    #           'show_auc_curve': True})